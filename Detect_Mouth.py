...
def detect_faces():
    with picamera.array.PiRGBArray(camera) as stream:
        ...
        for (x, y, w, h) in faces:
            ...
            # Define the region of interest (ROI) for the mouth within the detected face
            ...
            for (mx, my, mw, mh) in mouths:
                # Adjust mouth coordinates relative to the entire image
                cv2.rectangle(roi_color, (mx, my), (mx+mw, my+mh), (0, 255, 0), 2)
                break  # Only detect one mouth per face

        # Display the image with detected faces and mouths
        cv2.imshow('Face and Mouth Detection', image)
